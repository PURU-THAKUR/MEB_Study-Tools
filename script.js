...
// ================== IMPROVED SEARCH WITH DROPDOWN ==================
const searchInput = document.getElementById('toolSearch');
const searchWrapper = document.querySelector('.search-wrapper');

// Dropdown container create karo (agar already nahi hai to)
...
// Saare calculators ka ek combined index banao (naam + category + link)
let allCalculators = [];
for (const [category, names] of Object.entries(calculators)) {
    names.forEach(name => {
        allCalculators.push({
            name: name,
            category: category,
...
if (searchInput) {
    searchInput.addEventListener('input', function(e) {
        ...
        // Dropdown suggestions filter karo
        if (term.length < 2) {
        ...
        if (matches.length === 0) {
            searchDropdown.innerHTML = '<div class="search-item no-result">No calculators found</div>';
        ...
        matches.forEach(match => {
            html += `<a href="${match.url}" class="search-item">${match.name} <small>(${match.category})</small></a>`;
        });
        searchDropdown.innerHTML = html;
        searchDropdown.style.display = 'block';
    });

    // Dropdown ko hide karo jab focus bahar jaye
    ...
    // 🔥 ENTER KEY PRESS PAR PEHLA RESULT OPEN KARO
    searchInput.addEventListener('keydown', function(e) {
        if (e.key === 'Enter') {
            e.preventDefault(); // Form submit nahi hone denge
            
            const term = searchInput.value.toLowerCase().trim();
            if (term.length < 2) return;
            
            // Pehle dropdown check karo
            const firstItem = searchDropdown.querySelector('.search-item');
            if (firstItem && !firstItem.classList.contains('no-result')) {
                // Dropdown mein item hai to us par click karo
                window.location.href = firstItem.getAttribute('href');
                return;
            }
            
            // Agar dropdown mein nahi mila to exact match dhundho
            const exactMatch = allCalculators.find(calc => 
                calc.name.toLowerCase() === term
            );
            ...
            // Agar kuch nahi mila to first matching grid card par focus karo
            const firstVisibleCard = document.querySelector('.calc-card[style*="display: block"], .calc-card:not([style*="display: none"])');
            if (firstVisibleCard) {
                firstVisibleCard.scrollIntoView({ behavior: 'smooth', block: 'center' });
                firstVisibleCard.style.backgroundColor = 'var(--accent)';
                setTimeout(() => {
                    firstVisibleCard.style.backgroundColor = '';
                }, 1000);
            }
        }
    });
